...
# 3. Load diet
# ----------------------------
diet = pd.read_csv("mediterranean_prebiotic.csv")
...
solution.members.reset_index().to_csv(
    "E1_growth_rates_blend3_enriched_neutrient_glu_ins_pec.csv", index=False
)

with open("E1_summary_blend3_enriched_neutrient_glu_ins_pec.txt", "w") as f:
...
if solution.fluxes is not None:
    solution.fluxes.loc[:, solution.fluxes.columns.str.endswith("_m")]\
        .to_csv("E1_fluxes_blend3_enriched_neutrient_glu_ins_pec.csv")

print("Ã¢Å“â€¦ Blend 3 simulation complete!")
