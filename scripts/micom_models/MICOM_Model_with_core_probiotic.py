...
# ----------------------------
df = pd.read_csv("E1_synbiotic_community.csv")
...
# 3. Load diet
# ----------------------------
diet = pd.read_csv("mediterranean_synbiotic.csv")
...
solution.members.reset_index().to_csv(
    "E1_growth_rates_synbiotic.csv", index=False
)

with open("E1_summary_synbiotic.txt", "w") as f:
...
if solution.fluxes is not None:
    solution.fluxes.loc[:, solution.fluxes.columns.str.endswith("_m")]\
        .to_csv("E1_fluxes_synbiotic.csv")
